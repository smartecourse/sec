...
@section('onPage', 'Edit Konten Materi')
...
<div class="row">
    <div class="col-12">
        <div class="card shadow mb-4">
            ...
            <form class="card-body" method="POST" enctype="multipart/form-data" action="{{route('post-test-materi.update', $getDataPostTest->id)}}">
                @method('put')
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="materi">
                                ...
                                @foreach ($listMateri as $item)                        
                                    <option value="{{$item->id}}" @if($getDataPostTest->materi_id == $item->id) selected @endif>{{ strtoupper($item->tipe_kelas) }} {{$item->judul_kelas}} (materi {{$item->nama_materi}})</option>
                                @endforeach
                            </select>
                            @error('materi')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control  @error('judul_post_test') is-invalid @enderror" name="judul_post_test" placeholder="ex : Pengenalan Vocabulary" value="{{$getDataPostTest->judul_post_test}}" autocomplete="off">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control  @error('embed_link') is-invalid @enderror" name="embed_link" placeholder="ex : https://www.youtube.com/watch?v=SDROba_M42g" value="{{$getDataPostTest->embed_link}}" autocomplete="off">
                            @error('embed_link')
                                <div class="invalid-feedback">
                                    {{$message}}
                                </div>
