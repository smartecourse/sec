...
<div class="row">
    <div class="col-12">
        <div class="card shadow mb-4">
            ...
            <form class="card-body" method="POST" enctype="multipart/form-data" action="{{route('konten-materi.store')}}">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="materi">
                                ...
                                @foreach ($listMateri as $item)                        
                                    <option value="{{$item->id}}" @if(old('materi') == '{{$item->id}}') selected @endif>{{ strtoupper($item->tipe_kelas) }} {{$item->judul_kelas}} - {{$item->nama_jenis_kelas}} (materi {{$item->nama_materi}})</option>
                                @endforeach
                            </select>
                            @error('materi')
