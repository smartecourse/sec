    ...
    @foreach ($materiKonten as $kontenItem)
        @php
            ...
            $materiSelesai = \App\Models\MateriKontenSelesai::select('id', 'materi_konten_id')
                                                            ...
                                                            ->get();
            $materiLink[] = ['/dashboard/detail-kelas-aktif-video/'.$kelas->slug.'/'.$kontenItem->slug.'/'.$kontenItem->urutan.'/'.$noMateriTarget];
    ...
    <section class="section">
        ...
        <div class="d-flex justify-content-end bd-highlight mb-2">
            ...
            <div class="p-2 bd-highlight">
                <form action="{{ url('/dashboard/detail-kelas-aktif-video/'.$kelas->slug.'/'.$konten->slug.'/'.$konten->urutan) }}/check" method="POST">
                    @csrf
                    @if (count($materiLink) == Request::segment(6)[0])
                        <input type="hidden" name="segment" value="{{$materiLink[Request::segment(6) - 1][0]}}">
                    @elseif (count($materiLink) != Request::segment(6)[0])
                        <input type="hidden" name="segment" value="{{$materiLink[Request::segment(6)][0]}}">
                    @endif
                    <input class="btn icon icon-left btn-primary p-3" type="submit" value="Tandakan Selesai & Next Video">
                </form>
            </div>
        </div>
        <div class="row">
            ...
            <div class="card-body" style="line-height: 2rem">
                <div class="card-text">
                    <div class="embed-responsive embed-responsive-16by9">
                        <iframe src="https://www.youtube.com/embed/{{explode('=', $konten->konten)[1]}}">
                        </iframe>
                    </div>
                </div>
            </div>
    ...
    <div class="sidebar-wrapper active">
        ...
        <div class="sidebar-header text-center d-flex">
            ...
            <div class="align-self-center p-2">
                <p style="font-size: 20px; font-weight:600" class="">Kelas Video</p>
            </div>
        </div>
        <div class="sidebar-menu">
            <ul class="menu">
                ...
                <li class="sidebar-item {{ $isActive ? 'active' : '' }}">
                    <a href="{{ url('/dashboard/detail-kelas-aktif-video/'.$kelas->slug.'/'.$kontenItem->slug.'/'.$kontenItem->urutan.'/'.$noMateri) }}" class='sidebar-link'>
